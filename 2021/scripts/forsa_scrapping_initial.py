...
base_url_allensbach='https://www.wahlrecht.de/umfragen/forsa'
...
allensbach_concat.drop([432,537,789,1189,1190],axis=0, inplace=True)
...
for i in parties:
    allensbach_concat[i]=allensbach_concat[i].str.replace(',','.').str.replace('%','').str.replace('PIR 2.2 Sonst. 4.0','6.2') \
        .str.replace(r'k. A.', '-').str.replace('–', '-').str.replace('PIR 3 Sonst. 5 ', '8')
...
allensbach_concat['INSTITUT']='FORSA'
...
allensbach_concat.to_csv('C:/Users/Lutz/Desktop/Data Science/Politics/cleaned_polls/forsa.csv', index=False)