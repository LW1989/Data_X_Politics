...
base_url_allensbach='https://www.wahlrecht.de/umfragen/gms'
#range_list=list(range(1998,2008))
sub_url_years=['/projektion-2005', '/projektion-2009', '/projektion-2013', '/projektion-2017']
...
allensbach_concat.drop(['FW', 'PIRATEN'],axis=1, inplace=True)
allensbach_concat.drop([40, 92, 143,144,145, 192,193,194,227,228,229],axis=0, inplace=True)
...
for i in parties:
    allensbach_concat[i]=allensbach_concat[i].str.replace(',','.').str.replace('%','').str.replace('PIRATEN 2.2 Sonst. 4.0 ','6.2') \
        .str.replace(r'k. A.', '-').str.replace('–', '-').str.replace('PIRATEN 3 Sonst. 3 ', '6').astype('float')
...
allensbach_concat['BEFRAGTE']=allensbach_concat['BEFRAGTE'].str.replace(r'\*|\~|\.', '', regex=True)
allensbach_concat['BEFRAGTE']=allensbach_concat['BEFRAGTE'].str.replace(r'T • ', '', regex=True).astype('float')

allensbach_concat['INSTITUT']='GMS'
...
allensbach_concat.to_csv('C:/Users/Lutz/Desktop/Data Science/Politics/cleaned_polls/gms.csv', index=False)